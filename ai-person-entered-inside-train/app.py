...
# Use person entered inside train queue for this pipeline
VisionPipeline.queue_name = os.environ.get(Constants.QUEUE_NAME, Constants.PERSON_ENTERED_INSIDE_TRAIN_QUEUE)
...
def initialize_detectors() -> None:
    """
    Initializes the Train detection model and GPU manager.
    """
    try:
        model_path: str = os.environ.get(
            Constants.PERSON_ENTERED_INSIDE_TRAIN_MODEL_PATH,
            cfg.get_value_config(Constants.PERSON_ENTERED_INSIDE_TRAIN, Constants.PERSON_ENTERED_INSIDE_TRAIN_MODEL_PATH)
        )
        ...
        time.sleep(5)
        logger.info("Successfully initialized Person Entered Inside Train detection model and GPU manager.")
    except Exception as e:
        logger.error(f"Failed to initialize Person Entered Inside Train detection model: {str(e)}", exc_info=True)
        raise ModelLoadingError(f"Failed to initialize detectors: {str(e)}")
    
async def execute_VisionAiPipeline() -> None:
    """
    Executes the Vision AI Person Entered Inside Train detection pipeline.
    """
    try:
        logger.info("Launching Vision AI Person Entered Inside Train detection pipeline...")
