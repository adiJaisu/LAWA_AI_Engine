...
# Use restroom person tracking queue for this pipeline
VisionPipeline.queue_name = os.environ.get(Constants.QUEUE_NAME, Constants.RESTROOM_PERSON_TRACKING_QUEUE)
...
def initialize_detectors() -> None:
    """
    Initializes the Restroom Person Tracking model and GPU manager.
    """
    try:
        model_path: str = os.environ.get(
            Constants.RESTROOM_PERSON_TRACKING_MODEL_PATH,
            cfg.get_value_config(Constants.RESTROOM_PERSON_TRACKING, Constants.RESTROOM_PERSON_TRACKING_MODEL_PATH)
        )
        ...
        time.sleep(5)
        logger.info("Successfully initialized Restroom Person Tracking detection model and GPU manager.")
    except Exception as e:
        logger.error(f"Failed to initialize Restroom Person Tracking detection model: {str(e)}", exc_info=True)
        raise ModelLoadingError(f"Failed to initialize detectors: {str(e)}")
    
async def execute_VisionAiPipeline() -> None:
    """
    Executes the Vision AI Restroom Person Tracking detection pipeline.
    """
    try:
        logger.info("Launching Vision AI Restroom Person Tracking detection pipeline...")
