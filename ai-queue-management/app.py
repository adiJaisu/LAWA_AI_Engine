...
# Use queue management queue for this pipeline
VisionPipeline.queue_name = os.environ.get(Constants.QUEUE_NAME, Constants.QUEUE_MANAGEMENT_QUEUE)
...
def initialize_detectors() -> None:
    """
    Initializes the Queue Management model and GPU manager.
    """
    try:
        model_path: str = os.environ.get(
            Constants.QUEUE_MANAGEMENT_MODEL_PATH,
            cfg.get_value_config(Constants.QUEUE_MANAGEMENT, Constants.QUEUE_MANAGEMENT_MODEL_PATH)
        )
        ...
        time.sleep(5)
        logger.info("Successfully initialized Queue Management detection model and GPU manager.")
    except Exception as e:
        logger.error(f"Failed to initialize Queue Management detection model: {str(e)}", exc_info=True)
        raise ModelLoadingError(f"Failed to initialize detectors: {str(e)}")
    
async def execute_VisionAiPipeline() -> None:
    """
    Executes the Vision AI Queue Management detection pipeline.
    """
    try:
        logger.info("Launching Vision AI Queue Management detection pipeline...")
