...
# Use Bird Eye View queue for this pipeline
VisionPipeline.queue_name = os.environ.get(Constants.QUEUE_NAME, Constants.BIRD_EYE_VIEW_QUEUE)
...
def initialize_detectors() -> None:
    """
    Initializes the Bird Eye View model and GPU manager.
    """
    try:
        model_path: str = os.environ.get(
            Constants.BIRD_EYE_VIEW_MODEL_PATH,
            cfg.get_value_config(Constants.BIRD_EYE_VIEW, Constants.BIRD_EYE_VIEW_MODEL_PATH)
        )
        ...
        time.sleep(5)
        logger.info("Successfully initialized Bird Eye View detection model and GPU manager.")
    except Exception as e:
        logger.error(f"Failed to initialize Bird Eye View detection model: {str(e)}", exc_info=True)
        raise ModelLoadingError(f"Failed to initialize detectors: {str(e)}")
    
async def execute_VisionAiPipeline() -> None:
    """
    Executes the Vision AI Bird Eye View detection pipeline.
    """
    try:
