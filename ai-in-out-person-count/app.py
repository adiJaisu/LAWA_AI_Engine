...
# Use In_Out_Person_Count Queue
VisionPipeline.queue_name = os.environ.get(Constants.QUEUE_NAME, Constants.IN_OUT_PERSON_COUNT_QUEUE)
...
def initialize_detectors() -> None:
    ...
    """
    try:
        logger.info("Initializing in/out person count detection model and GPU manager...")
        # Since this usecase uses YOLO model we can just re-use a global path for YOLO or get it from env.
        # Fallback to LOITERING_DETECTION_MODEL_PATH for the default YOLO weights if needed:
        model_path: str = os.environ.get(
            Constants.IN_OUT_PERSON_COUNT_MODEL_PATH,
            cfg.get_value_config(Constants.IN_OUT_PERSON_COUNT, Constants.IN_OUT_PERSON_COUNT_MODEL_PATH)
        )
        ...
        time.sleep(5)
        logger.info("In/Out person count model and GPU manager initialized successfully.")
...
async def execute_VisionAiPipeline() -> None:
    ...
    """
    try:
        logger.info("Launching Vision AI In/Out person count detection pipeline...")
