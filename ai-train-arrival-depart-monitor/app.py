...
# Use Train Queue
VisionPipeline.queue_name = os.environ.get(Constants.QUEUE_NAME, Constants.TRAIN_ARRIVAL_DEPART_MONITOR_QUEUE)
...
def initialize_detectors() -> None:
    ...
    try:
        model_path: str = os.environ.get(
            Constants.TRAIN_ARRIVAL_DEPART_MONITOR_MODEL_PATH,
            cfg.get_value_config(Constants.TRAIN_ARRIVAL_DEPART_MONITOR, Constants.TRAIN_ARRIVAL_DEPART_MONITOR_MODEL_PATH)
        )
        ...
        time.sleep(5)
        logger.info("Train detection model and GPU manager initialized successfully.")
    except Exception as e:
        logger.error(f"Failed to initialize Train detection model: {str(e)}", exc_info=True)
        raise ModelLoadingError(f"Failed to initialize detectors: {str(e)}")
    
async def execute_VisionAiPipeline() -> None:
    """
    Executes the Vision AI Train detection pipeline.
    """
    try:
        logger.info("Launching Vision AI Train detection pipeline...")
