...
from src.business_cases.PersonEnteredInsideTrain import PersonEnteredInsideTrain
from src.Exception.Exception import FrameProcessingException, PersonEnteredInsideTrainException
    ...
    try:
        ...
        for idx, msg in enumerate(validated_msg_with_frames_and_metadatas):
            ...
            try:
                ...
                logger.error(f"Frame {idx}: Person entered inside train error: {str(e)}")
                continue
            except Exception as e:
                logger.error(f"Frame {idx}: Unexpected error during person entered inside train detection: {str(e)}", exc_info=True)
                continue
