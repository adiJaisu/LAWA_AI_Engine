...
def execute_crowd_density(validated_msg_with_frames_and_metadatas: List[Dict], detector: Any = None) -> List[Dict[str, Any]]:
        ...
        for idx, msg in enumerate(validated_msg_with_frames_and_metadatas):
            ...
            try:
                if camera_id not in VisionPipeline.camera_tracker:
                    VisionPipeline.camera_tracker[camera_id] = CrowdDensity()

                annotated_frame, crowd_alert, crowd_status = (
                ...
                msg[Constants.FRAME_METADATA][Constants.ALERT] = crowd_alert
