...
from src.business_cases.RestroomPersonTracking import RestroomPersonTracking
from src.Exception.Exception import FrameProcessingException, RestroomPersonTrackingException

logging_config = LoggingConfig()
logger = logging_config.setup_logging()

def execute_restroom_person_tracking(validated_msg_with_frames_and_metadatas: List[Dict], detector: Any = None) -> List[Dict[str, Any]]:
    try:
        ...
        for idx, msg in enumerate(validated_msg_with_frames_and_metadatas):
            ...
            try:
                if camera_id not in VisionPipeline.camera_tracker:
                    VisionPipeline.camera_tracker[camera_id] = RestroomPersonTracking(detector=detector)

                unannotated_frame, alert, status_dict = (
                ...
                if alert:
                    ...
                    cv2.imwrite(f"src/detected_frames/restroom_tracking_alert_{int(time())}_{idx}.jpg", unannotated_frame)
            ...
            except RestroomPersonTrackingException as e:
                logger.error(f"Frame {idx}: Restroom person tracking error: {str(e)}")
                continue
            except Exception as e:
                logger.error(f"Frame {idx}: Unexpected error during restroom person tracking: {str(e)}", exc_info=True)
                continue

        if alert_count > Constants.ZERO:
            logger.debug(f"Found alert for this batch for restroom person tracking. Count: {alert_count}")
            return processed_frames
        else:
            return []

    except FrameProcessingException as e:
        logger.critical(f"Critical error in restroom person tracking: {str(e)}", exc_info=True)
        raise
