...
from src.Exception.Exception import FrameProcessingException, LoiteringDetectionException
...
def execute_loitering_detection(validated_msg_with_frames_and_metadatas: List[Dict], detector: Any = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for idx, msg in enumerate(validated_msg_with_frames_and_metadatas):
            ...
            except LoiteringDetectionException as e:
                logger.error(f"Frame {idx}: Loitering detection error: {str(e)}")
                continue
