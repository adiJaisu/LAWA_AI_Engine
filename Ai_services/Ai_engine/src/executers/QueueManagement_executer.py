...
from src.business_cases.QueueManagement import QueueManagement
from src.Exception.Exception import FrameProcessingException, QueueManagementException

logging_config = LoggingConfig()
logger = logging_config.setup_logging()

def execute_queue_management(validated_msg_with_frames_and_metadatas: List[Dict], detector: Any = None) -> List[Dict[str, Any]]:
    try:
        ...
        for idx, msg in enumerate(validated_msg_with_frames_and_metadatas):
            ...
            try:
                if camera_id not in VisionPipeline.camera_tracker:
                    VisionPipeline.camera_tracker[camera_id] = QueueManagement(detector=detector)
                ...
                if alert:
                    ...
                    cv2.imwrite(f"src/detected_frames/queue_management_alert_{int(time())}_{idx}.jpg", unannotated_frame)
            ...
            except QueueManagementException as e:
                logger.error(f"Frame {idx}: Queue management error: {str(e)}")
                continue
            except Exception as e:
                logger.error(f"Frame {idx}: Unexpected error during queue management tracking: {str(e)}", exc_info=True)
                continue

        if alert_count > Constants.ZERO:
            logger.debug(f"Found alert for this batch for queue management. Count: {alert_count}")
            return processed_frames
        else:
            return []

    except FrameProcessingException as e:
        logger.critical(f"Critical error in queue management execution: {str(e)}", exc_info=True)
        raise
