...
from src.Exception.Exception import PredictionError, FrameProcessingException, PersonEnteredInsideTrainException
    ...
    def detect(self, frame: np.ndarray, rois: List[List[List[int]]], detector: Any) -> Tuple[np.ndarray, bool, Dict]:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Error during object tracking in PersonEnteredInsideTrain: {e}")
            ...
            try:
                if results.boxes is None or results.boxes.id is None:
                    ...
                    return frame, alert, {"platform_count": platform_count, "entered_count": entered_count, "bboxes": []}
                ...
                self.logger.error(f"Error processing detections in PersonEnteredInsideTrain: {e}")
                raise FrameProcessingException(f"Detection processing failed: {e}")
        except (PredictionError, FrameProcessingException) as e:
            raise PersonEnteredInsideTrainException(str(e))
        except Exception as e:
            self.logger.error(f"Unexpected error in PersonEnteredInsideTrain: {e}")
            raise PersonEnteredInsideTrainException(f"PersonEnteredInsideTrain failed: {e}")