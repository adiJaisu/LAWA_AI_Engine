...
from src.Exception.Exception import PredictionError, FrameProcessingException, TrainArrivalDepartMonitorException
    ...
    def detect(self, frame: np.ndarray, rois: List[List[List[int]]], detector: Any, timestamp: str = None) -> Tuple[np.ndarray, bool, Dict]:
        try:
            ...
            try:
                ...
                self.logger.error(f"Prediction failed in TrainArrivalDepartMonitor: {e}")
            ...
            try:
                ...
                self.logger.error(f"Error processing detections in TrainArrivalDepartMonitor: {e}")
                raise FrameProcessingException(f"Detection processing failed: {e}")
        except (PredictionError, FrameProcessingException) as e:
            raise TrainArrivalDepartMonitorException(str(e))
        except Exception as e:
            self.logger.error(f"Unexpected error in TrainArrivalDepartMonitor: {e}")
            raise TrainArrivalDepartMonitorException(f"TrainArrivalDepartMonitor failed: {e}")
