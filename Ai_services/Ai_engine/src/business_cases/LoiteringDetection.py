...
from src.Exception.Exception import PredictionError, FrameProcessingException, LoiteringDetectionException
    ...
    def detect(self, frame: np.ndarray, rois: List[List[List[int]]], detector: Any) -> Tuple[np.ndarray, bool, Dict]:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Error during object tracking in LoiteringDetection: {e}")
            ...
            try:
                ...
                self.logger.error(f"Error processing detections in LoiteringDetection: {e}")
                raise FrameProcessingException(f"Detection processing failed: {e}")
        except (PredictionError, FrameProcessingException) as e:
            raise LoiteringDetectionException(str(e))
        except Exception as e:
            self.logger.error(f"Unexpected error in LoiteringDetection: {e}")
            raise LoiteringDetectionException(f"LoiteringDetection failed: {e}")