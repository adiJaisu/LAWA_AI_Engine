...
def run_inference_task(model, secondary_model, task_data):
    ...
    response = {}
    try:
        if method_name == "make_prediction":
            results = model.make_prediction(frame, **kwargs)
        elif method_name == "make_prediction_with_tracking":
            results = model.make_prediction_with_tracking(frame, **kwargs)
        else:
...
def execute_AiInferenceService() -> None:
    """
    Executes the Ai Inference Service pipeline.
    """
    try:
