...
def initialize_segmentation_models() -> None:
    """
    ...
    try:
        ...
        model_path: str = os.environ.get(
            Constants.AI_INFERENCE_SEGMENTATION_MODEL_PATH,
            cfg.get_value_config(Constants.DEFAULT_ENVIRONMENT, Constants.AI_INFERENCE_SEGMENTATION_MODEL_PATH)
        )
...
def run_inference_task(model, secondary_model, task_data):
    ...
    response = {}
    try:
        if method_name == "make_prediction_segmentation":
            results = model.make_prediction_segmentation(frame, **kwargs)
        else:
        ...
        if results and hasattr(results[0], "masks") and results[0].masks is not None:
            if results[0].masks.xy is not None:
                masks = [mask.tolist() for mask in results[0].masks.xy]
        response = {"xyxy": xyxy, "ids": ids, "masks": masks}
    except Exception as e:
...
def execute_ai_inference_segmentation() -> None:
    """
    Executes the Ai Inference Segmentation Service pipeline.
    """
    try:
        logger.info("Launching AI Inference Segmentation Service...")
        ...
        AI_INFERENCE_SEGMENTATION_QUEUE = str(os.environ.get(
            Constants.AI_INFERENCE_SEGMENTATION_QUEUE,
            cfg.get_value_config(Constants.DEFAULT_ENVIRONMENT, Constants.AI_INFERENCE_SEGMENTATION_QUEUE)))
        credentials = pika.PlainCredentials(username, password)
        parameters = pika.ConnectionParameters(host=host, port=port, credentials=credentials, heartbeat=600)
        connection = pika.BlockingConnection(parameters)
        channel = connection.channel()
        channel.queue_declare(queue=AI_INFERENCE_SEGMENTATION_QUEUE)
        # Fair dispatch: give one message to a worker at a time
        channel.basic_qos(prefetch_count=1)
        # Note: auto_ack is False because we ack AFTER the GPU finishes
        channel.basic_consume(queue=AI_INFERENCE_SEGMENTATION_QUEUE, on_message_callback=on_request)
...
if __name__ == "__main__":
    try:
        execute_ai_inference_segmentation()
